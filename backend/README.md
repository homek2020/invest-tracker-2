...
The server listens on <http://localhost:3000> by default. Ensure the MongoDB environment variables are available so the application can connect on boot. The development runner is configured with `--esm`, so it is compatible with the ES module output generated by TypeScript.

...
The API listens on <http://localhost:3000> and exposes the `/api/v1` routes from the requirements document. The development command relies on [`ts-node-dev`](https://github.com/wclr/ts-node-dev) with the `--esm` flag so the TypeScript entrypoint runs in ES module mode without extra Node.js boilerplate. Ensure `MONGO_URI` and `MONGO_DB_NAME` are set in the environment before starting the server so it can establish the MongoDB connection.
